  ...
  <div>
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-5 mb-6">
      ...
      <div class="space-y-2 text-sm font-mono">
        <div class="flex justify-between items-center bg-gray-50 rounded p-3">
          ...
          <span class="text-gray-800">{{ serverRenderTime?.toISOString() }}</span>
        </div>
        <div class="flex justify-between items-center bg-gray-50 rounded p-3">
          <span class="text-gray-500">Client</span>
          <span class="text-gray-800">{{ clientRenderTime?.toISOString() }}</span>
        </div>
      </div>
    </div>
    <div class="bg-white border border-gray-200 rounded-lg p-5">
      ...
      <p class="text-sm text-gray-500 leading-relaxed mb-3">
        ...
        and <code class="font-mono text-gray-600">disableServerSideAuth: true</code>.
      ...
      <p class="text-sm text-gray-500 leading-relaxed">
        If the server render time stays the same across refreshes but the
        client render time updates, caching is working. The
